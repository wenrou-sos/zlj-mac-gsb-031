...
| 考察与常住 | 发心常住转入 **3–6 个月**考察期，**按月发起评议**：多位执事独立评分、评语修订留痕、缺席执事补评、每月阶段汇总；**评议次数具足、无未处理缺勤提醒、总平均分达标**方可羯磨转常住，并留存完整决策快照；不通过退回挂单 |
...
│   └── src/routes/     # monks / guadan / rooms / inspections / reviews / attendance / alerts / dashboard
...
- 3 位挂单、2 位考察期僧人
- 考察期僧人 **行简**：已汇总 2 轮月度评议（含僧值常济缺席补评 1 条、知客慧海评语修订留痕 1 次），第 3 轮评议中；**定空**：第 1 轮已汇总（均分 60），第 2 轮评议中
- 云水寮/静修楼共 11 个床位
...
| GET/POST | `/api/inspections` | 考察期列表（含评议进度）/转入考察 |
| POST | `/api/inspections/:id/pass` | 羯磨通过转常住（先校验门槛，留存决策快照） |
| POST | `/api/inspections/:id/fail` | 考察不通过（留存决策快照） |
| GET | `/api/reviews/inspections/:id` | 按月评议总览（轮次/评分/门槛） |
| POST | `/api/reviews/inspections/:id/rounds` | 发起新一轮月度评议 |
| POST | `/api/reviews/rounds/:id/scores` | 执事评分（已汇总轮次自动记为缺席补评） |
| PUT | `/api/reviews/scores/:id` | 修订评分/评语（留痕） |
| GET | `/api/reviews/scores/:id/revisions` | 评语修订历史 |
| POST | `/api/reviews/rounds/:id/summarize` | 阶段汇总（冻结人数/平均分/缺勤数） |
| GET | `/api/reviews/inspections/:id/gate` | 羯磨门槛检查 |
| GET | `/api/reviews/inspections/:id/decisions` | 羯磨决策快照 |
